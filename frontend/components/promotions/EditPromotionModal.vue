  ...
  >
    <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div v-if="form.id" class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 mb-6">
          <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">當前統計</h4>
          <div class="grid grid-cols-3 gap-4 text-center">
            <div>
              <div class="text-lg font-bold text-blue-600 dark:text-blue-400">
                {{ formatNumber(selectedPromotion?.click_count || 0) }}
              </div>
              <div class="text-xs text-gray-500 dark:text-gray-400">總點擊數</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div class="text-xs text-gray-500 dark:text-gray-400">已發放獎勵</div>
            </div>
          </div>
        </div>
...
const handleSubmit = async () => {
  ...
  const result = await updatePromotion(form.value.id, form.value)

  if (result.success) {
    emit('updated', result.promotion)
    closeModal()
  } else {
    console.error('Failed to update promotion:', result.error)
  }
}
